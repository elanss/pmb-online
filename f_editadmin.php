...
$p_rektor		= isset($_POST['rektor']) ? $_POST['rektor'] : NULL;
...
if ($tb_act == "Edit") {
	...
	if ($p_nama != "" && $p_alamat != "" && $p_ta != "" && $p_rektor != "") {
		$q_edit_universitas	= mysql_query("UPDATE t_universitas SET 
								nama_universitas = '$p_nama',
								alamat = '$p_alamat',
								tahun_ajaran = '$p_ta',
								rektor = '$p_rektor' $logo,
								beranda = '$p_beranda',
								prosedur = '$p_prosedur'								
								WHERE admin = '$p_user'");
		
		if ($q_edit_universitas) {
...
<article class="module width_full">
	<header><h3>Data universitas Pengguna</h3></header>
		<div class="module_content">
		
		<?php		
		// ================ TAMPILKAN DATANYA =====================//
		echo "<table border='1' class='data'><tr><th width='10%'>ID</th><th width='40%'>Nama Universitas</th><th width='20%'>Username</th><th width='30%'>Control</th></tr>";
		$q_universitas 	= mysql_query("SELECT * FROM t_universitas") or die (mysql_error());
		$j_data 	= mysql_num_rows($q_universitas);

		if ($j_data == 0) {
			echo "<tr><td id='tengah' colspan='3'>-- Tidak Ada Data --</td></tr>";
		} else {
			$no = 1;
			while ($a_universitas = mysql_fetch_array($q_universitas)) {
				echo "<tr>
				<td id='tengah'>$no</td>
				<td>$a_universitas[0]</td>
				<td id='tengah'>".$_SESSION['username']."</td>
				<td id='tengah'><a href='?p=f_editadmin&mod=edit&id=$a_universitas[0]'>Edit</a>
				</tr>";
				$no++;
			}
		}
		...
		if ($mod == "edit") {
			...
			$q_edit_universitas	= mysql_query("SELECT * FROM t_universitas WHERE admin = '".$_SESSION['username']."' AND flag = '1'");
			
			$a_edit_universitas	= mysql_fetch_array($q_edit_universitas);
			
			$universitas_0 = $a_edit_universitas[0];
			$universitas_1 = $a_edit_universitas[1];
			$universitas_2 = $a_edit_universitas[2];
			$universitas_3 = $a_edit_universitas[3];
			$universitas_4 = $a_edit_universitas[4];
			$universitas_5 = $a_edit_universitas[5];
			$universitas_6 = $a_edit_universitas[6];
			$universitas_7 = $a_edit_universitas[7];
			$universitas_8 = $a_edit_universitas[8];
...
<article class="module width_full" <?php echo $display;?>>
	<header><h3><?php echo $view;?> Data universitas</h3></header>
		<div class="module_content">
		<form action="?p=f_editadmin" method="post" id="ft_universitas" enctype="multipart/form-data">
		<table class="tbl_form">
			<?php
			cInput("Nama Universitas", "nama", "30", $universitas_0);
			?>
			<tr><td>Username</td><td><input type="text" name="user" readonly value="<?php echo $universitas_8; ?>"></td></tr>
			<tr><td>Ganti Passwod Jadi</td><td><input type="password" name="pass" value=""></td></tr>
			<?php
			cInput("Alamat", "alamat", "60", $universitas_1);
			cInput("Tahun Ajaran", "ta", "20", $universitas_2);
			cInput("Nama Rektor", "rektor", "30", $universitas_3);
			?>
