...
if ($p_tombol == "DAFTAR") {
	...
	if ($p_nama == "" || $p_jk == "" || $p_agama == "" || $p_tmp_lahir == "" 
		...
		echo "<h4 class='alert_error'>Jurusan Pilihan 1 dan 2 masih KOSONG<span id='close'>[<a href='#'>X</a>]</span></h4>";
			...
			$q_cek_sek_asal = mysql_query("SELECT * FROM t_sekolah WHERE sekolah = '$p_asal_skl' AND status = '$p_stat_skl'");
			$j_cek_sek_asal = mysql_num_rows($q_cek_sek_asal);
			
			//INSERT DATA Sekolah Asal Otomatis jika belum ada */
			if ($j_cek_sek_asal < 1) {
				$q_sek_baru = mysql_query("INSERT INTO t_sekolah VALUES ('', '$p_asal_skl', '$p_stat_skl', '$p_sc_alamat', '$p_kepsek')");
	...
	if ($p_nama == "" || $p_jk == "" || $p_agama == "" || $p_tmp_lahir == "" 
		...
		/* PENGECEKAN Simpan Sekolah Asal jika BELUM ADA */
		$q_cek_sek_asal = mysql_query("SELECT * FROM t_sekolah WHERE sekolah = '$p_asal_skl' AND status = '$p_stat_skl' AND alamat = '$p_sc_alamat' AND kepsek = '$p_kepsek'");
		$j_cek_sek_asal = mysql_num_rows($q_cek_sek_asal);
